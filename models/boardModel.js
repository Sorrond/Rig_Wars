...
module.exports.moveBoatsById = async function (gamebit_id, tile_i, tile_j) {
  try {
    let sql = "select (roomuser_user_id = (Select roomuser_id from roomuser WHERE roomuser_user_id = $1 AND roomuser_room_id = $2)) from roomuser INNER JOIN turn ON turn_roomuser_id = roomuser_id Where turn_id = (Select max (turn_id) from turn INNER JOIN roomuser on roomuser_id = turn_roomuser_id WHERE roomuser_room_id = $2)";
    let result = await pool.query(sql, [req.userId, 1]);
    if (result) {
